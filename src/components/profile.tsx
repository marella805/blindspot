...
export function Profile({ data, isFresh, savedAnswers }: Props) {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        borderTop: `1px solid ${on ? 'var(--blue-ink-500)' : 'var(--border)'}`,
                        borderBottom: `1px solid ${on ? 'var(--blue-ink-500)' : 'var(--border)'}`,
                        borderRight: `1px solid ${on ? 'var(--blue-ink-500)' : 'var(--border)'}`,
      ...
      <div className="screen animate-enter" style={{ maxWidth: 660 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <button
            ...
            onClick={async () => {
              if (isFresh) {
                // Already saved in goNext — just navigate
                router.push('/log')
                return
              }
              setSaving(true)
          ...
          >
            ...
            {isFresh ? 'Start making decisions' : saving ? 'Saving…' : 'Save profile'}
          </button>
          <button
            onClick={restart}
  ...
  return (
    <div className="screen animate-enter">
      ...
      <div className="card">
        <div className="card-section">
          <div className="label-xs" style={{ marginBottom: 10 }}>Achievements</div>
          <div className="achievement-row">
            {ACHIEVEMENTS.map((a, i) => {
              const isUnlocked = a.unlocked(data)
              return (
                <div
                  key={a.id}
                  className={`achievement-pill ${isUnlocked ? 'unlocked' : 'locked'}`}
                  style={{ animationDelay: `${i * 40}ms` }}
                  title={isUnlocked ? a.label : `${a.label} (locked)`}
                >
                  {isUnlocked
                    ? <i className={a.icon} style={{ fontSize: 13 }} />
                    : <i className="ph ph-lock" style={{ fontSize: 12 }} />
                  }
                  {a.label}
                </div>
              )
